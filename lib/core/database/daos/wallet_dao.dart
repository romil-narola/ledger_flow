...
class WalletDao extends DatabaseAccessor<AppDatabase>
    with _$WalletDaoMixin, ScopedBusinessDao {
  WalletDao(super.db);
  ...
  Future<List<WalletAccount>> getAllWallets() {
    return (select(walletAccounts)
          ..where((w) =>
              w.businessId.equals(currentBusinessId) & w.isActive.equals(true))
  ...
  Future<WalletAccount?> getWalletById(int id) {
    return (select(walletAccounts)
          ..where(
              (w) => w.id.equals(id) & w.businessId.equals(currentBusinessId)))
        .getSingleOrNull();
  }
  ...
  Future<double> getTotalBalance() async {
    final query = selectOnly(walletAccounts)
      ...
      ..where(walletAccounts.businessId.equals(currentBusinessId) &
          walletAccounts.isActive.equals(true));
  ...
  Future<List<LedgerEntry>> getWalletLedger({
    ...
    query.where((l) {
      Expression<bool> expr = l.walletAccountId.equals(walletId) &
  ...
  Future<int> insertWallet(WalletAccountsCompanion wallet) {
    return into(walletAccounts)
        .insert(wallet.copyWith(businessId: Value(currentBusinessId)));
  }
